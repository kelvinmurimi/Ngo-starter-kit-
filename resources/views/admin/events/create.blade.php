    ...
    <div class="card">
        <div class="flex-1 p-8">
            ...
            <div x-data="{ submitting: false }" class="mt-6">
                <x-form x-on:submit="submitting = true" action="{{ route('admin.events.store') }}" method="POST"
                    class="space-y-6" enctype="multipart/form-data">
                    @csrf
                    {{-- title --}}
                    <div>
                        <x-form.input name='title' label='Event Title' />
                    </div>
                    {{-- description /excerpt --}}
                    <div>
                        <x-form.textarea name='description' label='Event Description' cols='10' rows='10'>
                        </x-form.textarea>
                    </div>
                    {{-- content --}}
                    <div>
                        <x-form.textarea name='location' label='Event Location' cols='10' rows='10'>
                        </x-form.textarea>
                    </div>
                    {{-- select dates --}}
                    <x-form.input name='start_time' type='datetime-local' label='Start Time' />
                    <x-form.input name='end_time' type='datetime-local' label='End Time' />
                    {{-- featured Image --}}
                    <x-form.input name="featured_image" type="file"></x-form.input>
                    {{-- organizer --}}
                    <x-form.input name='organizer' label='Event Organizer' />
                    {{-- submit button --}}
                    <div>
